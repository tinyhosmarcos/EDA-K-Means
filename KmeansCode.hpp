...
bool view_clusters = true;

const int W = 900, H = 900;

const int n_colors_clusters = 13;

Color clusters[n_colors_clusters] = { Color(0,0,0),
                      Color(255,0,0),
                      Color(0,255,0),
                      Color(0,0,255),
                      Color(255, 153, 51),
...
struct DataPoint {
  ...
  void show(RenderWindow *win) {
    point.setFillColor((view_clusters) ? clusters[cluster_id] : clusters[0]);
    win->draw(point);
  }
};

vector<double> map_2d(string line, char sep) {
...
void pick_random_centroids(vector<DataPoint> &dataset, vector<DataPoint> &centroids, size_t k) {
  for (size_t i = 1; i <= k; ++i) {
    ...
    dataset[new_centroid].cluster_id = i % n_colors_clusters;
    ...
    try {
      if (dataset_points.is_open()) {
        ...
        while (!dataset_points.eof() && n_data < 100000) {
          ...
          vector<double> data = map_2d(data_line, ' ');
